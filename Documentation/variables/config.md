...
| tectonic_custom_cacertificates | List of paths to SSL CA Certificates you would like trusted on Kubernetes nodes, in PEM-encoded x509 format.<br><br>These certificates will be added to the root of trust on both master and worker nodes via the update-cacertificates-rehash systemd service enabled on every boot.<br><br>Example: tectonic_custom_cacertificates = [   "~/fake-ssl/authority1.crt",   "~/fake-ssl/authority2.crt",   "~/fake-ssl/authority3.crt" ] | string | `<list>` |
...
| tectonic_registry_cache_image_repo | (offline) image repo to pull tectonic registry cache image from | string | - |
| tectonic_registry_cache_image_tag | (offline) image tag for tectonic registry cache image (empty string unless tectonic_registry_cache_rkt_protocol = "docker://") | string | `` |
| tectonic_registry_cache_rkt_insecure_options | (offline) rkt insecure options to set when pulling tectonic registry cache image. | string | - |
| tectonic_registry_cache_rkt_protocol | (offline) rkt image protocol string to pull tectonic registry cache image. | string | - |
| tectonic_rkt_image_protocol | (offline) Protocol rkt will use when pulling images from registry.<br><br>Example: `docker://` | string | - |
| tectonic_rkt_insecure_options | (offline) Comma-separated list of insecure options for rkt. Example: `image,tls` | string | - |
