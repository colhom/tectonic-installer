...
resource "template_dir" "tectonic" {
  ...
  vars {
    ...
    prometheus_config_reloader_image      = "${var.container_images["tectonic_prom_cfg_reloader"]}"
    ...
    container_linux_update_operator_repo  = "${replace(var.container_images["container_linux_update_operator"], var.image_re,"$1")}"
    ...
    prometheus_repo                       = "${replace(var.container_images["prometheus"], var.image_re,"$1")}"
